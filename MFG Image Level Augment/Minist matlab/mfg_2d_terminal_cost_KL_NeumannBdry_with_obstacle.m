...
M1 =32%space discretization [0,1.0]
M2 = 32
N=32 %time discretization
ht=1.0/(N); %time step
hx1 = 1.0/M1;
hx2= 1/M2;
m = zeros(M1,M2,2,N); %
...
for i = 1:M1
    ...
    for j = 1:M2
        x2 =(j-1)*hx2;
        rho0(i,j) = 1/sigma/sqrt(2*pi)*1/sigma/sqrt(2*pi)*exp(-0.5*(((x1-0.5)^2+(x2-0.5)^2)/(sigma^2)));      
        Q(i,j) = 20.0*1/sigmaq/sqrt(2*pi)*exp(-0.5*(((x1-0.65)^2+(x2-0.65)^2)/(sigmaq^2)));
    end
end
% sigma_rho_T =0.1;
for k = 1:8
    xx1 = cos((k-1)*pi/4)*0.4+0.5;
    xx2 = sin((k-1)*pi/4)*0.4+0.5;
for i = 1:M1
    x1 =(i-1)*hx1;
    for j = 1:M2
        x2 =(j-1)*hx2 ;
...
phi = 0*ones(M1,M2,N);%zeros(N,M);
phi1 = 0*c_terminal*ones(M1,M2);
...
%     record_cost(itr,:) = mfg_cost_2D( rho,m,phi,phi1,rho0,rho1,rho_target,M1,M2,N,c_kl,c_terminal,Q);
    ...
    for i = 1:M1
        for j = 1:M2
            fun = @(x) myfun(x,rho1(i,j),beta(i,j),tau5*c_terminal); 
